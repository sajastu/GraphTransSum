...
for se in [args.set]:
    ...
    for j, f in tqdm(enumerate(glob.glob(os.path.join(PT_DIRS, se + '*.pt'))), total=len(glob.glob(os.path.join(PT_DIRS, se + '*.pt')))):
        ...
        for inst_idx, instance in enumerate(instances):
            ...
            sent_labels = instance['src_sent_labels']
    ...
    x = collections.Counter(avg_sents_len.values())
    plt.bar([x for x in range(1, len(list(dict(sorted(x.items())).values()))+1)], list(dict(sorted(x.items())).values()), color='green',
            width=0.5)
    # plt.hist(, bins=15)  # density=False would make counts
    plt.ylabel('Count')
    plt.xlabel('Data')
    plt.savefig(f'{se}.pdf')
