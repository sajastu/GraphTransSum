...
from models.data_loader import load_dataset
from models.graphSum_builder import ExtSummarizer
# from models.summarizer_builder import ExtSummarizer
...
def test_ext(args, device_id, pt, step, intro_cls=False, intro_sents_cls=False, intro_top_cls=False, pick_top=False):
    ...
    if pick_top:
        ...
        trainer.validate_rouge_baseline(test_iter_fct, step, is_test=True)
...
def train_single_ext(args, device_id):
    ...
    trainer = build_trainer(args, device_id, model, optim, train_iter_fct=train_iter_fct)

    run_wandb = wandb.init(
        project=(args.model_path.split('/')[-1] if args.model_path[-1] != '/' else args.model_path.split('/')[-2]) ,
        group="DDP",  # all runs for the experiment in one group
    )

    trainer.train(train_iter_fct, args.train_steps, valid_iter_fct=val_iter_fct, wandb=run_wandb)

    test_ext(args, device_id, args.model_path + '/model_step_' + str(trainer.best_val_step) + '.pt', trainer.best_val_step)