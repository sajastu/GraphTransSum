    ...
    def __init__(self, large, model_name, temp_dir, finetune=False):
        ...
        if model_name == 'bert':
            ...
            if large:
                ...
                self.model = LongformerModel.from_pretrained('allenai/longformer-large-4096', config=self.model.config)
            else:
...
class BertVanilla(nn.Module):
    def __init__(self, model_name, temp_dir, large=False, finetune=True):
    ...
    def forward(self, x, segs, mask_src, is_inference=False):
        if (not is_inference):
