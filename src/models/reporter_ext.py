...
#         self.rl = rl
#
#     def set_overall_recall(self, recall1, recall2, recall3):
#         self.top_sents_recall_sect_stat = recall1
#         self.top_sents_recall_normal = recall2
#         self.top_sents_recall_sect_eq = recall3
#
...
class ReportMgrBase(object):
    ...
    def report_training(self, step, num_steps, epoch_counter, learning_rate,
...
class ReportMgr(ReportMgrBase):
    ...
    def _report_training(self, step, num_steps, epoch, learning_rate,
                         report_stats, wandb=None):
        """
        See base class method `ReportMgrBase.report_training`.
        """
        report_stats.output(step, num_steps, epoch,
                            learning_rate, self.start_time, wandb)
    ...
    def _report_step(self, lr, step, train_stats=None, valid_stats=None):
        ...
        """
        if train_stats is not None:
            self.log('Train xent: %g ==> xent_sent: %g, xent_rg: %g' % (train_stats.xent(), train_stats.xent_sent(), train_stats.xent_rg()))
        ...
        if valid_stats is not None:
            self.log('Validation xent: %g at step %d ==> xent_sent: %g, xent_rg: %g' % (valid_stats.xent(), step,  valid_stats.xent_sent(), valid_stats.xent_rg()))
...
class Statistics(object):
    ...
    * elapsed time
    ...
    def __init__(self, loss=0, n_docs=0, loss_rg=0, loss_sent=0):
        self.loss = loss
        self.loss_rg = loss_rg
        self.loss_sent = loss_sent
        self.n_docs = n_docs
        self.start_time = time.time()
    ...
    def xent_rg(self):
        ...
        if (self.n_docs == 0):
            return 0
        return self.loss_rg / self.n_docs
    ...
    def output(self, step, num_steps, epoch, learning_rate, start, wandb=None):
        """Write out statistics to stdout.
        Args:
           step (int): current step
           n_batch (int): total batches
        ...
        if self.xent_sent() !=0 :
            logger.info(
                ("Step %s (epoch %s); xent: %4.2f, xent_sent: %4.2f, xent_rg: %4.2f; " +
                 "lr: %7.7f; %3.0f docs/s; %6.0f sec")
                % (step_fmt,
                   epoch,
                   self.xent(),
        ...
        wandb.log(
            {
                "train/loss": self.xent(),
                "train/loss_sent": self.xent_sent(),
                "train/loss_rg": self.xent_rg(),
                "train/epoch": step,
