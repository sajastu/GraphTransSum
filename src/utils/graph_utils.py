...
class Node():
    def __init__(self,s_sent_idx, s_token_idx, e_sent_idx=None, e_token_idx=None, s_node_txt=None, e_node_txt=None):
...
class Graph():
    ...
    def set_nodes_and_edges(self, bert_nodes_len):
        ...
        self.node_family = [str(pre_id) for pre_id in range(int(self.node_family[0].split('-')[0]))] + self.node_family + [str(post_id) for post_id in range(int(self.node_family[-1].split('-')[-1]) + 1, bert_nodes_len)]
    ...
    def _prepare_graph(self, bert_embeds, clss, clss_mask, src_mask, src, id):
        """
        ...
        graph_embs = torch.zeros(bert_embeds.size(0), len(self.node_family), bert_embeds.size(2)).cuda()
        clss_graph = torch.zeros(clss.size(0), clss.size(1)).cuda()
        sentence_num = 0
        for k, v in idx_mapping.items():
            mean_indices = k
            embedded_into = v
            token_embed = torch.index_select(bert_embeds, 1, torch.tensor(eval(f'[{mean_indices.replace("-", ",")}]')).cuda()).mean(dim=1).unsqueeze(0)
